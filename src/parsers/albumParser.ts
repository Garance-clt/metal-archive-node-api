...
import type { Album, Track } from "../models/Album.js";

export function parseAlbum(html: string, id: string): Album {
  const $ = load(html);
...
/* ------- helper ------- */
function parseTracklist($: CheerioAPI): {
  tracks: Track[];
  total: string | null;
} {
  const res: Track[] = [];
  ...
  $("table.table_lyrics tr").each((_, tr) => {
    ...
    if (!cells.eq(1).length) return; // skip spacer / total rows
