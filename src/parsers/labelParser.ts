...
function tryLabelLogo(id: string): string | null {
  try {
    return buildImageUrl(id, "_label", "gif");
  } catch {
    try { return buildImageUrl(id, "_label", "png"); } catch { return null; }
  }
}
...
export function parseLabel(html: string, id: string): Label {
  ...
  const statsDd: Record<string, string> = {};  // raw HTML for links
  ...
  if (subDd) {
    load(subDd)("a[href]").each((_, a) => {
      ...
      if (subId) subLabels.push({ id: subId, name: el.text().trim() });
    });
  }

  // Parent label
  let parentLabel: SubLabel | null = null;
  const parentDd = statsDd["Parent label"] ?? "";
  if (parentDd) {
    const pa = pickAnchor(parentDd);
    if (pa) parentLabel = pa;
  }
