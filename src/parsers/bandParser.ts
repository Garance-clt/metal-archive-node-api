...
function extractImg($: CheerioAPI, anchorSelector: string) {
  const anchor = $(anchorSelector);
  const fromImg =
    anchor.find("img").attr("src") ?? anchor.find("img").attr("data-src");
  if (fromImg) return resolveUrl(fromImg);
  ...
  if (!members.length) {
    $("#band_tab_members_all table.lineupTable tr.lineupRow").each((_, tr) => {
      ...
      const statusText = $(tr)
        ...
        .text()
        .toLowerCase();
      let status: Member["status"] = "past";
      if (statusText.includes("current")) status = "current";
