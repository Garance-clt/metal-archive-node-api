...
function grabSectionTextAndReadMore($, title) {
    ...
    if (!$h2.length)
        return { text: null, readMoreUrl: null };
...
function extractContributionsForBandBlock($scope, $block) {
    ...
    rows.each((_, tr) => {
        ...
        const cover = releaseId && /^\d+$/.test(releaseId) ? buildCoverUrl(releaseId) : null;
        list.push({
            releaseId,
            title,
            year,
...
export function parseArtist(html, id) {
    ...
    // Photo
    const photo = $("#member_sidebar a.image#artist").attr("href") ??
    ...
    const diedRaw = pick("R.I.P.") ?? ageLine.match(/R\.I\.P\.\s*:\s*([^()]+)/i)?.[1] ?? null;
    const died = diedRaw?.trim() || null;
    const diedISO = toISODate(died);
    const diedOf = pick("Died of") ?? null;
    // Sections Overview
    const { text: bio, readMoreUrl: bioReadMoreUrl } = grabSectionTextAndReadMore($, "Biography");
    const { text: trivia, readMoreUrl: triviaReadMoreUrl } = grabSectionTextAndReadMore($, "Trivia");
    const statuses = {
